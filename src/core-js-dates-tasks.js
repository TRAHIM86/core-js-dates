...
function getNextFriday(date) {
  ...
  const numDay = new Date(date).getDay();
  const untilFriday = (5 - numDay + 7) % 7 || 7;
  return new Date(curentDate.setDate(curentDaY + untilFriday));
}
...
function getCountDaysInMonth(month, year) {
  const dateWith0 = new Date(year, month, 0);
  const lastDay = dateWith0.getDate();
  return lastDay;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const mcsInDay = 24 * 60 * 60 * 1000;
  return (date2 - date1) / mcsInDay + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return curentDate >= startDate && curentDate <= endDate;
}
...
function getWeekNumberByDate(date) {
  ...
  const curentDate = date - newYear;
  const mlsInWeek = 1000 * 60 * 60 * 24 * 7;
  return Math.floor(curentDate / mlsInWeek) + 1;
}
...
function getNextFridayThe13th(date) {
  ...
  const differenceToFriday = (friday - day + 7) % 7;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const parseDate = (strOfDate) => {
    const [day, month, year] = strOfDate.split('-');
    return new Date(year, month - 1, day);
  };

  const startStamp = parseDate(period.start).getTime();
  const endStamp = parseDate(period.end).getTime();
  let curentStamp = startStamp;
  const dayStamp = 1000 * 60 * 60 * 24;
  const days = [];

  while (curentStamp <= endStamp) {
    for (let i = 1; i <= countWorkDays; i += 1) {
      if (curentStamp > endStamp) break;

      const d = new Date(curentStamp);

      const day = d.getDate().toString().padStart(2, '0');
      const month = (d.getMonth() + 1).toString().padStart(2, '0');
      const year = d.getFullYear().toString().padStart(2, '0');
      days.push(`${day}-${month}-${year}`);
      curentStamp += dayStamp;
    }

    curentStamp += dayStamp * countOffDays;
  }
  return days;
}
